...
<body>
	...
	<p>The cart has {$cart_count} product(s). <a href="{$YD_SELF_SCRIPT}?do=empty">Empty
    the cart</a>. </p>
	...
	<p>{$id}. <b>{$detail[name]}</b> - $ {$detail[price]} - {$detail[quantity]} items
	    available <br />
 ...
 |
	<a href="{$YD_SELF_SCRIPT}?do=modify&id={$id}&quantity=0">set to none </a> 	</p>
