<?php
	...
	*/
	...
	class YDDatabaseObject extends YDBase {
		...
		function getRelationValues( $prefix=true, $relation='' ) {
			...
			foreach ( $relations as $relation ) {
				...
				$f_var = $rel->getForeignVar();
				$values = $this->$f_var->getValues( false, false, $prefix ? $f_var . '_' : '' );
				...
				if ( $rel->isManyToMany() ) {
					...
					$c_var = $rel->getCrossVar();
					$values = $this->$c_var->getValues( false, false, $prefix ? $c_var . '_' : '' );
	...
	class YDDatabaseObject_Relation extends YDDatabaseObject_Properties {
		 ...
		 *
		 *  @param $name  The foreign join type. E.g. INNER, LEFT, RIGHT, etc.
		 */	
		 ...
		 *
		 *  @param $name  The join type. E.g. INNER, LEFT, RIGHT, etc.
		 */		
