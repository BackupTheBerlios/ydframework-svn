<?php
	...
	/**
	 *	This is the global error handler for the Yellow Duck Framework and will take care of all error handling.
	 *
	...
	function YDErrorHandler( $errno, $errstr, $errfile, $errline ) {
		...
		// Honour the defined level of error reporting set either in php.ini or with error_reporting()
		if ( ! ( $errno & error_reporting() ) ) { return; }

		// Output the error text
		echo( '<p style="background-color: lightyellow;"><font color="red" size="-1">' );
		echo( '<b>' . $errortypes[ $errno ] . '</b><br><i>' . $errfile . ' (line ' . $errline . ')</i><br>' );
		echo( $errstr . '</font></p>' );
