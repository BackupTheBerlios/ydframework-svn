...
class OPMLCreator extends FeedCreator {
	...
	function createFeed() {     
		...
		$feed.= "	<head>\n";
		$feed.= "		<title>Generated by FeedCreator - www.bitfolge.de</title>\n";
		$feed.= "	</head>\n";
		$feed.= "	<body>\n";
		//<outline type="rss" title="" description="" xmlUrl="http://xxx/rss.xml" htmlUrl="http://xxx/" /> 
		for ($i=0;$i<count($this->items);$i++) {
			$feed.= "		<outline type=\"rss\" title=\"".htmlspecialchars(strip_tags(strtr($this->items[$i]->title,"\n\r","  ")))."\"";
			//$feed.= " description=\"".htmlspecialchars($this->items[$i]->description)."\"";
			$feed.= " xmlUrl=\"".htmlspecialchars($this->items[$i]->link)."\"";
			$feed.= "/>\n";
		}
		$feed.= "	</body>\n";
		$feed.= "</opml>\n";
